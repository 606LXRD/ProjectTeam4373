...
# счет строк
def Count():
    cur.execute("SELECT userid FROM анализы")
...
combo = Combobox(ws)
cur.execute("SELECT фамилия FROM keys")

combo['values'] = cur.fetchall()
combo.current(1)
combo.place(x=22, y=35)

btn = Button(ws, text="    НОВЫЙ    ", command=Delete)
btn.place(x=200, y=33)

btn = Button(ws, text="    Ввод    ", command=Input)
